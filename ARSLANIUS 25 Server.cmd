...
if exist "%root_path%\Setting And System Files" (
    if not exist "%root_path%\Settings And Server Files" (
        ren "%root_path%\Setting And System Files" "Settings And Server Files"
    )
)
...
cls 
echo ======================================================================================================================
echo                                                 Loading ARSLANIUS 25 Server
echo         %current_build%
echo         Kernel: BarOS 21.1
echo ======================================================================================================================
...
echo.
echo File: \Settings And Server Files\kernel.dll
echo.
...
echo OS_NAME = ARSLANIUS 25 Server > "%reg_path%"
...
:kernel_ok
findstr /i /c:"BarOS AUTHORITY\\SERVER =" "%kernel_path%" >nul
if !errorlevel! EQU 0 (
    cls
    color 0f
    echo ======================================================================================================================
...
set "system_hash="
for /f "tokens=2 delims==" %%a in ('findstr /i /c:"SERVER =" "%kernel_path%" 2^>nul') do set "system_hash=%%a"
...
if not "!system_hash!"=="!expected_system_hash!" (
    ...
    echo Info: SERVER_ACCOUNT_HASH_MISMATCH
...
if not exist "%reg_path%" (
    ...
    echo.
    echo File: \Settings And Server Files\REG.cfg
    echo.
    echo Press [R] to Repair
    echo Press [Enter] to Exit
    echo.
...
if /i NOT "%current_user%"=="BarOS AUTHORITY\SERVER" (
    echo [ ERROR ] Only SERVER can change attributes.
    goto cmd_loop
)
...
echo System: Help, Logout, Lock, sudo, cls, Shutdown, ver, fmx, whoami, reboot, clean, service, events, restore-point, restore, passwd, ls, cd, cat, ren, mkdir, touch, bsod, backup, backup-restore
echo Admin: CreatedFolder, adduser, deluser, alert, report, reset, reboot_to_recovery, chattr
goto cmd_loop
...
if /i "%bsod%"=="1" (
   ...
   echo.
   echo For support, visit: https://github.com/Armsoup/ARSLANIUS_Server/issues
   pause
   goto repair
)
cls
...
if /i "%bsod%"=="2" (
   ...
   echo *** File: \Settings And Server Files\kernel.dll
   echo.
   echo SERVER_ACCOUNT_HASH_MISMATCH - Someone's been playing with kernel.dll in Notepad, huh?
   echo The SERVER account hash doesn't match. Nice try, hacker.
   echo.
   echo Technical information:
...
if /i "%bsod%"=="5" (
   ...
   echo Someone tried to create 'BarOS AUTHORITY\SERVER' in kernel.dll.
